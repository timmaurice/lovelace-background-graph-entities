...
- **Highly Customizable:**
  ...
  - **Honest Gaps:** Enable `show_gaps` to break the line while an entity is `unavailable` or `unknown`, rather than implying a measurement that never happened.
...
| `show_gaps`        | boolean | `false`      | Draws a break in the line for periods where the entity was `unavailable` or `unknown`, instead of carrying the last known value across them. Useful for sensors where a flat line would falsely imply a measurement was taken.                                                                                                                                       |
...
| `points_per_hour`  | number  | `1`          | The number of time buckets per hour. The card calculates a time-weighted average for each bucket and fills in any gaps with the last known value to create a continuous graph (unless `show_gaps` is enabled). Higher values provide more detail but may impact performance.                                                                                         |
...
| `auto_icon_color`            | boolean        | `false`                   | When `true`, the icon color follows the graph color of the chosen history point (see `auto_icon_color_source`) using smooth interpolation between `color_thresholds` to match the graph's visual fade. Falls back to `icon_color` when no history is available. |
...
entities:
  ...
  # auto_icon_color makes the icon follow the same threshold color
  # auto_icon_color_source makes the icon reflect the worst-case (max) over history
  ...
  # Display the peak temperature observed over the history window with a label
...
#### 4. Independent Extra Value (graph stays on the main entity)

Unlike `graph_entity`, `extra_value_entity` is a plain extra value — it doesn't change what's
...
Extra values — both this one and `graph_entity`'s `show_graph_entity_state` — render on a second
line beneath the main value, so long labels don't cover the graph. Row height is unchanged. In
`tile_style` they stay on one line, separated by a middle dot.
...
entities:
  - entity: sensor.kitchen_temperature
    name: Kitchen
    extra_value_entity: sensor.kitchen_humidity
```

Add `extra_value_name` to label it — a string of your own, or `true` to reuse the extra entity's
friendly name:
...
entities:
  - entity: sensor.kitchen_temperature
    name: Kitchen
    extra_value_entity: sensor.kitchen_humidity
    extra_value_name: Humidity # renders as "21.3 °C · Humidity: 54 %"
```
